...
@intercept_warnings
def call_openrouter_or_agentrouter(messages: list, url: str, api_key: str, model: str, is_agentrouter: bool = False) -> dict:
...
@intercept_warnings
def call_anthropic(messages: list, model: str) -> dict:
...
def call_api(messages: list, provider: str = None, model: str = None) -> dict:
    ...
    if prov == "hackclub":
        target_model = model or "claude-3-5-sonnet-20241022" # Hackclub default
        return call_openrouter_or_agentrouter(
            messages,
    ...
    if prov == "openrouter":
        ...
        return call_anthropic(messages, model=target_model)
